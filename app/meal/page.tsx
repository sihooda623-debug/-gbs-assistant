              ...
              {Object.entries(ALLERGEN_MAP).map(([code, name]) => {
                ...
                return (
                  <button key={code}
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium border-2 transition-colors ${
                      userAllergens.includes(n) ? "border-danger-400 bg-danger-50 text-danger-700" : "border-gray-200 text-gray-500"
                    }`}>
                    {name}
                  </button>
                );
          ...
          <>
            ...
            {(favPrefs.length > 0 || userAllergens.length > 0) && (
              <div className="flex gap-3 text-xs text-gray-400">
                ...
                {userAllergens.length > 0 && <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-danger-100 border border-danger-300 inline-block"/>알레르기 주의</span>}
              </div>
            )}
          </>
        )}
